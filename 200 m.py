...
class MeetLogicProcessor:
    ...
    def execute_initial_seeding(self):
        self.pipeline.cursor.execute("SELECT stu_id, name, class, house, dob, height, weight FROM school_data WHERE run_200 = 1")
        runners = self.pipeline.cursor.fetchall()
        
        if len(runners) == 0:
            return "[ERROR] No students found running the 200m. Check your Excel file!"

        pools = {"O": [], "A": [], "B": [], "C": []}
        for stu_id, name, cls, house, dob, height, weight in runners:
            score = self.calculate_legacy_score(dob, height, weight)
            div = "O" if score >= 100 else "A" if score >= 88 else "B" if score >= 76 else "C"                                                                                                                        #for changing the scores required for a division
        ...
        log_view = "=== STAGE 1: EXTERNAL EXCEL INITIAL SEEDING LOG (200M) ===\n"
        ...
        for div, pool in pools.items():
            ...
            while len(pool) > 0:
                ...
                for idx, runner in enumerate(lanes, start=1):
                    if runner:
                        self.pipeline.cursor.execute('INSERT INTO trials_active (stu_id, event, heat, lane, name, house, class, division, track_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', (runner["id"], '200m', heat_idx, idx, runner["name"], runner["house"], runner["class"], div, 0.0))
...
class ModernMeetTerminal:
    def __init__(self, window, pipeline, engine):
        ...
        tk.Label(header_frame, text="TRACK & FIELD BRACKET ENGINE(200M)", bg="#0f4c75", fg="#ffffff", font=("Segoe UI", 16, "bold")).pack()
            ...
            for div, runners in finalists_by_div.items():
                if runners:
                    ...
                    for lane, (name, cls, house, time) in enumerate(runners[:6], start=1):
                        self.pipeline.cursor.execute("INSERT INTO finals_roster VALUES (?, ?, ?, ?, ?, ?)", ('200m', div, lane, name, house, time))
...
# ==========================================================================================
    def trigger_house_points(self):
        ...
        points_scale, medals = [10, 6, 4], ["1st Place", "2nd Place", "3rd Place"]                                                                                                                                                                                             #for final scores fo the houses
